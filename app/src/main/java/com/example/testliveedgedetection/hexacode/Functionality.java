...
//        Bitmap buffer = BitmapFactory.decodeFile(inputFile.getAbsolutePath(),bmOptions);
    ...
    public final boolean DrawCode(List<Area> areas, int w, int h) {
...
//C# TO JAVA CONVERTER TODO TASK: There is no preprocessor in Java:
        ...
        Bitmap bitmap = Bitmap.createBitmap(params.getWidth(), params.getHeight(), Bitmap.Config.ARGB_8888);


        GhCode ghCode = new GhCode(context, bitmap, params, areas);
        viewGroup.addView(ghCode);
        try {
            Code code = ConvertHXtoDC(bitmap);
        } catch (IOException e) {
            e.printStackTrace();
        }
